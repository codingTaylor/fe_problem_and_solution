/**
* fork from https://github.com/mmastrac/jquery-noclickdelay.git
*/
...
(function (window, document) {
  ...
  function agentTest(agent, window) {
    var regs = [/(iPhone|iPad|iPod)/];
    var type = Object.prototype.toString.call(agent);
  ...
  function addDocListener() {
    if (window.noClickDelay.hasDocListener) {
      return;
    }
    ...
    window.noClickDelay.hasDocListener = true;
  }

  window.noClickDelay = function (agent) {
    if (!agentTest(agent, window)) {
      return;
    }
    addDocListener();
    return function (elem) {
      ...
      elem.addEventListener('touchstart', function (e) {
        ...
        elem.classList.add(CONFIG.PRESSED_CLASS);
        ...
        this.__onTouchMove = function (e) {
          ...
          if (withinDistance(touch.clientX, touch.clientY, location[0], location[1],
            ...
            elem.classList.remove(CONFIG.PRESSED_CLASS);
          }
        };

        document.body.addEventListener('touchmove', this.__onTouchMove);
      });

      elem.addEventListener('touchcancel', function () {
        // elem.removeClass(CONFIG.PRESSED_CLASS);
        elem.classList.remove(CONFIG.PRESSED_CLASS);
        document.body.removeEventListener('touchmove', this.__onTouchMove);
      });

      elem.addEventListener('touchend', function (e) {
        if (elem.classList.contains(CONFIG.PRESSED_CLASS)) {
          // elem.removeClass(CONFIG.PRESSED_CLASS);
          elem.classList.remove(CONFIG.PRESSED_CLASS);
...
window.onload = function () {
  ...
  window.noClickDelay()(document.querySelector('.button'));
}
